...
# MODEL = 'pxlksr/opencodeinterpreter-ds:latest'
# MODEL = 'pxlksr/opencodeinterpreter-ds:33b-Q4_K_M'
...
@pr = @client.create_pull_request(
  ...
  MAIN_BRANCH,
  branch_name,
  "Optimize: #{issue.title} [#{MODEL}]",
  "Optimizations for ##{issue_number} using #{MODEL}\n\nPath: #{path}\nRequests: #{agent_prompt}",
  draft: true
)

@client.add_labels_to_an_issue(@repo, @pr.number, ['optimize', 'open-source-llms'])
...
prompt = <<-EOS
  Please, refactor the rspec file I send to you by using `let_it_be`

  It only creates a record ONCE, which is much faster.
...
messages << { role: "user", content: target_file }
...
loop do
  ...
  response = conn.post('/v1/chat/completions', payload, { 'Content-Type' => 'application/json' })
  ...
  if action_index
    ...
    if action == "run_rspec"
      binding.irb
      code_begin_index = lines.find_index { _1 =~ /^# frozen.*/ }
      code_end_index = code_begin_index + lines[code_begin_index..].find_index { _1 =~ /^end/ } + 1
